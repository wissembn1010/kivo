(() => {
  ...
  document.querySelector("#cf-toggle-password").addEventListener("click", (event) => {
    ...
    event.currentTarget.textContent = visible ? __("Show") : __("Hide");
  });
  form.addEventListener("submit", async (event) => {
    event.preventDefault();
    if (password.value !== confirm.value) {
      message.textContent = __("Passwords do not match."); message.className = "cf-message cf-error"; return;
    }
    if (!form.reportValidity() || button.disabled) return;
    button.disabled = true; button.textContent = __("Creating workspace…");
    try {
      ...
      password.value = ""; confirm.value = "";
      message.textContent = __("This link is invalid, expired, or could not be completed. Request a new verification email.");
      message.className = "cf-message cf-error";
      button.disabled = false; button.textContent = __("Create my Kivo workspace");
    }
  });
})();
